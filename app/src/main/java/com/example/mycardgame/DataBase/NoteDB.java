...
public class NoteDB  extends SQLiteOpenHelper {
    ...
    private static final String DATABASE_NAME = "noteTable";
...
// Closing database connection
    ...
    public void deleteSong(String songName,File myFile)
    {
        ...
        for (int i = 0; i < noteDBStructuresArray.size(); i++) {
            File file = new File(dir,noteDBStructuresArray.get(i).getSongPathKey());
     ...
     **/
    public NoteDBStructure getNoteTextStructure(String songPath){
        NoteDBStructure noteDBStructure = new NoteDBStructure();
...
//        }
        if (cursor.moveToFirst()) {
            do {
                noteDBStructure.setSongPathKey(cursor.getString(cursor.getColumnIndex(COLUMN_SONG_NAME_PATH)));
                noteDBStructure.setSongName(cursor.getString(cursor.getColumnIndex(COLUMN_SONG_NAME)));
            } while (cursor.moveToNext());
        }
